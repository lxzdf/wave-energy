...
def rk4(time, x_fu, v_fu, x_zh, v_zh, c_pto):
    # 浮子的速度(L)、加速度(K)----[t+Δt时刻]
    fu_K1 = acc_fu(time, x_fu, v_fu, x_zh, v_zh, c_pto)
    fu_L1 = v_fu                                                                                                # 使用t时刻的速度
    ...
    zh_K3 = acc_zh(time+0.5*dt, x_fu+0.5*dt*fu_L2, v_fu+0.5*dt*fu_K2, x_zh+0.5*dt*zh_L2, v_zh+0.5*dt*zh_K2,c_pto)
    zh_L3 = v_zh + 0.5*dt*zh_K2

    fu_K4 = acc_fu(time+dt, x_fu+dt*fu_L3, v_fu+dt*fu_K3, x_zh+dt*zh_L3, v_zh+dt*zh_K3, c_pto)
    fu_L4 = v_fu + fu_K3*dt
    zh_K4 = acc_zh(time+dt, x_fu+dt*fu_L3, v_fu+dt*fu_K3, x_zh+dt*zh_L3, v_zh+dt*zh_K3, c_pto)
    zh_L4 = v_zh + zh_K3*dt
...
if __name__ == '__main__':
    ...
    evaluated = {}
    for x in np.arange(0, 100001, 1000):
        P = cal_power(x)
        evaluated[x] = P
        if P > max_power:
            max_power = P
    ...
    for i in range(int(max(0, best_c - 10)), int(min(100000, best_c + 10)) + 1, 1):
        if i in evaluated:
            continue
        P = cal_power(i)
        evaluated[i] = P
    ...
    print("对平均输出功率:{}".format(max_power))

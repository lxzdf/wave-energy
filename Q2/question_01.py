...
# 等价的方法
def deriv(run_time, y, c_pto):
    x_fu = y[0]
    v_fu = y[1]
    x_zh = y[2]
    v_zh = y[3]
    a_fu = acc_fu(run_time, x_fu, v_fu, x_zh, v_zh, c_pto)
    a_zh = acc_zh(run_time, x_fu, v_fu, x_zh, v_zh, c_pto)

    return np.array([v_fu, a_fu, v_zh, a_zh], dtype=float)

# 四阶龙格库塔法主要做的事情是，给定当前时刻的t以及当前时刻的系统状态，使用四阶龙格库塔法将此状态推进一个步长，得到下一时刻的状态
def rk4_step(t, y, h, c_pto):
    k1 = deriv(t, y, c_pto)
    k2 = deriv(t+0.5*h, y+0.5*h*k1, c_pto)
    k3 = deriv(t+0.5*h, y+0.5*h*k2, c_pto)
    k4 = deriv(t+h, y+h*k3, c_pto)
...
def cal_power(c_pto):
    ...
    for zz in range(steps):
        y = rk4_step(time, y, dt, c_pto)
        time = time+dt

        if time >= power_start:
            pp = c_pto*pow((y[1]-y[3]), 2)
